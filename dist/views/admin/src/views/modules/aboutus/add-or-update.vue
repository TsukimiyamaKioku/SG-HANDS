		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.picture1" label="图片1" prop="picture1">
					...
					<img v-else class="upload-img" style="margin-right:20px;"  v-for="(item,index) in ruleForm.picture1.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.picture2" label="图片2" prop="picture2">
					...
					<img v-else class="upload-img" style="margin-right:20px;"  v-for="(item,index) in ruleForm.picture2.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.picture3" label="图片3" prop="picture3">
					...
					<img v-else class="upload-img" style="margin-right:20px;"  v-for="(item,index) in ruleForm.picture3.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		var validateIdCard = (rule, value, callback) => {
			if(!value){
				...
				callback(new Error("Please enter corrent NRIC"));
			} else {
				callback();
			}
		};
		...
		var validateMobile = (rule, value, callback) => {
			if(!value){
				...
				callback(new Error("Please enter correct mobile no."));
			} else {
				callback();
			}
		};
		...
		var validateIntNumber = (rule, value, callback) => {
			if(!value){
				...
				callback(new Error("Please enter integer number"));
			} else {
				callback();
			}
		};
		return {
			...
			rules: {
				...
				content: [
					{ required: true, message: 'Content cannot be blank', trigger: 'blur' },
				],
				picture1: [
				],
				picture2: [
    ...
    info(id) {
      this.$http({
        ...
        //
        let reg=new RegExp('../../../upload','g')//g means all
        this.ruleForm.content = this.ruleForm.content.replace(reg,'../../../nodejsv6o62/upload');
        } else {
