...
<div :style='{"width":"90%","padding":"30px 0px 40px","margin":"10px auto","position":"relative","background":"rgba(255,255,255,.3)"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="头像" v-if="type!='cross' || (type=='cross' && !ro.avatarurl)" prop="avatarurl">
            <file-upload
            tip="点击上传头像"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' class="upload" v-else label="头像" prop="avatarurl">
                ...
                <img v-else class="upload-img" style="margin-right:20px;"  v-for="(item,index) in ruleForm.avatarurl.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="用户名" prop="nickname">
            <el-input v-model="ruleForm.nickname" 
                placeholder="用户名" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="评论内容" prop="content">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="评论内容"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="回复内容" prop="reply">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="回复内容"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('discussgongyitonggao/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('discussgongyitonggao/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('discussgongyitonggao/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
