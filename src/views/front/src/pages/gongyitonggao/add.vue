...
<div :style='{"width":"90%","padding":"30px 0px 40px","margin":"10px auto","position":"relative","background":"rgba(255,255,255,.3)"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="Title" prop="gongyibiaoti">
            <el-input v-model="ruleForm.gongyibiaoti" 
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="Cover" v-if="type!='cross' || (type=='cross' && !ro.fengmian)" prop="fengmian">
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' class="upload" v-else label="Cover" prop="fengmian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;"  v-for="(item,index) in ruleForm.fengmian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="Publish time" prop="fabushijian" >
              <el-date-picker
                  format="yyyy  MM  dd "
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="Cover" prop="jianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="Summary"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0"}' label="Content" prop="gonggaoneirong">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('gongyitonggao/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('gongyitonggao/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('gongyitonggao/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
