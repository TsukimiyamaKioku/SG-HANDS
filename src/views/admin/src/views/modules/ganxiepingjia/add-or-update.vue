<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'" label="评价编号" prop="pingjiabianhao">
					<el-input v-model="ruleForm.pingjiabianhao" placeholder="评价编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.pingjiabianhao" label="评价编号" prop="pingjiabianhao">
					<el-input v-model="ruleForm.pingjiabianhao" placeholder="评价编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="虚拟礼物" prop="xuniliwu">
					<el-select :disabled="ro.xuniliwu" v-model="ruleForm.xuniliwu" placeholder="请选择虚拟礼物" multiple>
						<el-option
							v-for="(item,index) in xuniliwuOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="虚拟礼物" prop="xuniliwu">
					<el-input v-model="ruleForm.xuniliwu"
						placeholder="虚拟礼物" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="date" v-if="type!='info'" label="评价时间" prop="pingjiashijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.pingjiashijian" label="评价时间" prop="pingjiashijian">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="评价内容" prop="pingjianeirong">
					<el-input
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.pingjianeirong" label="评价内容" prop="pingjianeirong">
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='xuniliwu'){
							this.ruleForm.xuniliwu = obj[o];
							this.ro.xuniliwu = true;
							continue;
						}
						if(o=='pingjiashijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.xuniliwu = this.ruleForm.xuniliwu.split(",");
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ganxiepingjia/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ganxiepingjiaCrossAddOrUpdateFlag = false;
						 this.parent.search();
             ...
             this.ruleForm.xuniliwu = this.ruleForm.xuniliwu.join(",");
			 this.$http({
			   url: `ganxiepingjia/${!this.ruleForm.id ? "save" : "update"}`,
			   method: "post",
			   data: this.ruleForm
